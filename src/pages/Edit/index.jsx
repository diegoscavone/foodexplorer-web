...
export function Edit() {
  ...
  useEffect(() => {
    async function fetchDish() {
      try {
        ...
        setName(dishes.title)
        setCategory(dishes.category_id)
        setDescription(dishes.description)
        const formattedPrice = `R$ ${String(dishes.price).replace('.', ',')}`

        setPrice(formattedPrice)
        setIngredients(dishes.ingredients.map(ingredient => ingredient.name))
      } catch (error) {
        console.error(error)
      }
    }
  ...
  async function handleUpdateDish() {
    ...
    try {
      const response = await api.put(`/dishes/${id}`, updateDish)
      ...
      toast.update(toastId.current, {
        render: 'Erro ao atualizar prato.',
        type: 'error',
        isLoading: false,
        progress: false,
  ...
  async function handleRemoveDish(id){
    toastId.current = toast.loading('Aguarde...Removendo prato...')

    try {
      await api.delete(`/dishes/${id}`)
      toast.update(toastId.current, {
        render: 'Prato removido com sucesso!',
        type: 'success',
        isLoading: false,
        progress: false,
        bodyStyle: { background: '#04D361' },
    ...
    <Container>
      ...
      <Content>
        ...
        <Form>
          ...
          <div className="col-1">
            <ButtonText title="Excluir Prato" onClick={() => handleRemoveDish(id)} />
            <Button
